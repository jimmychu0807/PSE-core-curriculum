...
describe("MerkleTree", function () {
  ...
  it("Insert two new leaves and verify the first leaf in an inclusion proof", async function () {
    ...
    const { proof: proof2, publicSignals: ps2 } = await groth16.fullProve(
      input2,
      "circuits/circuit_js/circuit.wasm",
      "circuits/circuit_final.zkey"
     );

    const calldata2 = await groth16.exportSolidityCallData(proof2, ps2);
    ...
    const vInput = argv2.slice(8);

    expect(await merkleTree.verify(a2, b2, c2, vInput)).to.be.true;
  });
});
